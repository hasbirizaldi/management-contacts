...
import { userDetail, userUpdateProfileName, userUpdateProfilePassword } from "../../lib/api/UserApi";
...
export default function UserProfile() {
  ...
  async function handleSubmitProfile(e) {
    e.preventDefault();
    const response = await userUpdateProfileName(token, { name });
    const responseBody = await response.json();
    console.log(responseBody);
  ...
  async function handleSubmitPassword(e) {
    ...
    const response = await userUpdateProfilePassword(token, { password });
    const responseBody = await response.json();
    console.log(responseBody);
    ...
    if (response.status === 200) {
      ...
      const allErrors = responseBody.errors
  ...
  useEffectOnce(() => {
    fetchUserData().then(() => console.log("User detail fetch successfully"));
  });
  return (
    <>
          ...
          <div className="p-6">
            ...
            <form onSubmit={handleSubmitProfile}>
              ...
              <div className="mt-6">
                <button
                  type="submit"
                  className="cursor-pointer w-full bg-gradient text-white py-3 px-4 rounded-lg hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 focus:ring-offset-gray-800 transition-all duration-200 font-medium shadow-lg transform hover:-translate-y-0.5 flex items-center justify-center"
                >
