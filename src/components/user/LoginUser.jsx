...
export default function LoginUser() {
  ...
  async function handleSubmit(e) {
    ...
    if (response.status === 200) {
      // save dulu token ke localstorage
      ...
      await alertError("Username or Password wrong!");
    }
  }
  return (
    <div className="animate-fade-in bg-gray-800 bg-opacity-80 p-8 rounded-xl shadow-custom border border-gray-700 backdrop-blur-sm w-full max-w-md">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="text-center text-sm text-gray-400">
          ...
          <Link to="/register" className="ml-2 text-blue-400 hover:text-blue-300 font-medium transition-colors duration-200">
            Sign up
          </Link>
        </div>
        <div className="text-center text-sm text-gray-400 mt-3">
          Go to
          <Link to="/" className="ml-2 text-blue-400 hover:text-blue-300 font-medium transition-colors duration-200 ml-2">
            Dashboard
          </Link>
        </div>
      </form>
